...
from AM import fban5  as app, AM, fban6 as ass
from AM.Helper.db.chatsdb import add_afk, is_afk, remove_afk, get_readable_time
from pyrogram.enums import MessageEntityType
import time, re
from config import BOT_USERNAME, OWNER_ID, FBAN, UNFBAN, SUDO_USERS, APPEND, Doneapped

AMBOT = 6601999645
RAI = 6297223639

@ass.on_message(filters.command(["afk","brb"], prefixes=["."]) & filters.user(RAI))
@app.on_message(filters.command(["afk","brb"], prefixes=["."]) & filters.user(AMBOT))
async def active_afk(_, message: Message):
        ...
        try:
            ...
            if afktype == "text":
                send = await message.reply_text(
                    f"{message.from_user.mention} ɪs ʙᴀᴄᴋ ᴏɴʟɪɴᴇ ᴀɴᴅ ᴡᴀs ᴀᴡᴀʏ ғᴏʀ {seenago}",
            ...
            send = await message.reply_text(
                f"**{message.from_user.mention}** ɪs ʙᴀᴄᴋ ᴏɴʟɪɴᴇ",
                disable_web_page_preview=True,
            )
    ...
    await message.reply_text(f"{message.from_user.mention} ɪs ɴᴏᴡ ᴀғᴋ!")
